...
wget https://wordpress.org/wordpress-5.0.3.zip -O /tmp/wordpress.zip;
cd /tmp/;
...
sed -i "s/'DB_NAME', 'example.com_db'/'DB_NAME', 'wordpress'/g" /tmp/wordpress/wp-config.php;
...
sed -i "0,/example.com/s/example.com/$wp_salt/" /tmp/wordpress/wp-config.php;
done
